...
const statsBlock = `<div className="grid grid-cols-1 md:grid-cols-3 gap-6">` + middleParts[0].trim();
...
const historyEndIndex = historyContent.indexOf('</Card>');
...
function addActionToCard(cardHtml) {
  return cardHtml.replace(/<Card title="([^"]+)">/, '<Card title="$1" action={<div className="text-slate-300 cursor-grab active:cursor-grabbing hover:text-indigo-500 transition-colors cursor-pointer z-50 pointer-events-auto" title="Arraste para reordenar" onPointerDown={(e) => e.stopPropagation()}><GripVertical size={20} /></div>}>');
}
